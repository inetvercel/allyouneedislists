...
function SectionRule({ label, href, icon }: { label: string; href: string; icon?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between mb-5 pb-3 border-b border-white/[0.08]">
      <h2 className="flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-gray-100">
        {icon}{label}
      </h2>
      <Link href={href} className="flex items-center gap-1 text-[10px] font-bold text-[#E63946] hover:text-red-400 uppercase tracking-wide transition-colors">
...
function EditorialCard({ post, size = 'md' }: { post: PostCardType; size?: 'lg' | 'md' | 'sm' }) {
  ...
  return (
    <Link href={href} className="group flex flex-col overflow-hidden rounded-none bg-[#1a1a1a] hover:bg-[#222] transition-colors duration-200 h-full border border-white/[0.06]">
      <div className="relative overflow-hidden bg-[#111]" style={{ aspectRatio: '16/9' }}>
        {img ? <Image src={img} alt={post.title} fill className="object-cover group-hover:scale-[1.03] transition-transform duration-500" />
              : <div className="absolute inset-0 bg-gradient-to-br from-gray-800 to-gray-700" />}
        {cat && <span className="absolute top-2.5 left-2.5 px-1.5 py-0.5 bg-[#E63946] text-white text-[8px] font-black uppercase tracking-[0.12em]">{cat.name}</span>}
      </div>
      <div className="p-3.5 flex flex-col flex-1">
        {cat && <span className="text-[9px] font-black uppercase tracking-[0.12em] text-[#E63946] mb-1">{cat.name}</span>}
        <h3 className={`font-bold text-gray-100 leading-snug line-clamp-2 group-hover:text-[#E63946] transition-colors ${size === 'lg' ? 'text-lg mb-2' : 'text-[0.9rem] mb-1'}`}>{post.title}</h3>
        {post.excerpt && size === 'lg' && <p className="text-gray-500 text-sm leading-relaxed line-clamp-2 mb-3 flex-1">{stripHtml(post.excerpt)}</p>}
        <time className="text-[10px] text-gray-600 mt-auto">{formatDate(post.date)}</time>
      </div>
    </Link>
  )
}
...
function RowCard({ post, index }: { post: PostCardType; index: number }) {
  ...
  return (
    <Link href={href} className="group flex items-center gap-3 py-3 border-b border-white/[0.07] last:border-0 hover:bg-white/[0.04] -mx-2 px-2 rounded transition-colors">
      <span className="text-[13px] font-black text-[#E63946] w-5 text-center flex-shrink-0 tabular-nums">{index + 1}</span>
      <div className="relative flex-shrink-0 w-[62px] h-[44px] rounded overflow-hidden bg-[#222]">
      ...
      <div className="flex-1 min-w-0">
        ...
        <h4 className="text-[0.82rem] font-bold text-gray-300 leading-snug line-clamp-2 group-hover:text-[#E63946] transition-colors mt-0.5">{post.title}</h4>
      </div>
    </Link>
  )
}
...
function InlineCard({ post }: { post: PostCardType }) {
  ...
  return (
    <Link href={href} className="group flex gap-3 items-center py-3 border-b border-white/[0.07] last:border-0 hover:bg-white/[0.04] -mx-1 px-1 rounded transition-colors">
      <div className="relative flex-shrink-0 w-[72px] h-[50px] rounded overflow-hidden bg-[#222]">
        {img && <Image src={img} alt={post.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />}
      </div>
      <div className="flex-1 min-w-0">
        {cat && <span className="text-[8px] font-black uppercase tracking-[0.12em] text-[#E63946]">{cat.name}</span>}
        <h4 className="text-[0.82rem] font-semibold text-gray-300 leading-snug line-clamp-2 group-hover:text-[#E63946] transition-colors mt-0.5">{post.title}</h4>
      </div>
    </Link>
  )
}
    ...
    return (
      <div className="bg-[#0f0f0f] min-h-screen">
        <div className={`${W} mx-auto px-4 py-10`}>
          <div className="border-b border-white/[0.08] pb-3 mb-7">
            <h2 className="text-sm font-black uppercase tracking-widest text-gray-100">All Lists — Page {page}</h2>
          </div>
  ...
  return (
    <div className="bg-[#0f0f0f] min-h-screen">
      ...
      {(S.entertainment?.length ?? 0) > 0 && (
        <section className="bg-[#161616] mt-12 py-10">
          <div className={`${W} mx-auto px-4`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
              ...
              <div className="lg:col-span-2 divide-y divide-white/[0.06]">
      ...
      {/* ── BROWSE BY CATEGORY ── */}
      <section className="mt-12 py-14 bg-[#161616] border-t border-white/[0.06]">
        <div className={`${W} mx-auto px-4`}>
